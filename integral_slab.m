function [phi,phiL] = integral_slab(L,SigT,SigS,Q,N,M)
% function phi = integral_slab(L,SigmaT,SigmaS,Q,N,M)
...
% uncollided flux
phiL(:,1) = von_neumann(L,N,QL,SigT);
% collided fluxes
for i = 1:M
   QL          = SigS*phiL(:,i);     	   % compute new source
   phiL(:,i+1) = von_neumann(L,N,QL,SigT); % get next flux term
end
...
end % function integral_slab

function phi = von_neumann(L,N,Q,SigT)
% function phi = von_neumann(L,N,Q,SigT)
...
%   been used.
% 
%   The flux is computed as follows:
