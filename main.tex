%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LaTeX Template for 22.106 Notes
% J. Roberts
% Spring, 2011
% Note, requires LATEST version of memoir class 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%---------------------------------------------------------------%
% PREAMBLE
%---------------------------------------------------------------%

\documentclass[ letterpaper, 11pt, twoside, openany ]{memoir}
% NOTE: The option ``oneside'' is for one-sided printing and happens
%       to reduce several blank pages that would otherwise arise.
\usepackage{bm}
% This gives a bit more white space...possibly desirable.
%\OnehalfSpace

\usepackage{format106}
\usepackage{hyperref} 
\newcommand{\cov}{{$\, \mathbf{C}_{\sigma\sigma} \,$}}

\newcommand{\keff}{$k_{\text{eff}}$ }
\newcommand{\ie}{\textit{i.e. }}
\newcommand{\eg}{\textit{e.g. }}
%---------------------------------------------------------------%
% DOCUMENT
%---------------------------------------------------------------%

\begin{document}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Insert Title
\thispagestyle{empty}
\titleTH

\frontmatter
\addtocontents{toc}{\protect\thispagestyle{plain}}
\tableofcontents*

\mainmatter 

\part{Nuclear Data}

\include{microscopic_interactions}
\include{macroscopic_interactions}
\include{shielding_and_moderation}
\include{neutron_detection_and_spectroscopy}
\include{nuclear_data}
\include{r_matrix_theory}
\include{neutron_thermalization}
\include{scattering_laws}

\part{Monte Carlo Transport}

\include{probability_and_statistics}
\include{monte_carlo_methods}
\include{collision_physics}
\include{tallies_and_uncertainties}
\include{variance_reduction}
\include{criticality}
\include{criticality_safety}
% 
\part{Deterministic Transport} 

\include{transport_equation}
\include{boundary_conditions}
\include{analytical_solutions}
\include{the_integral_form}
\include{collision_probability_method}
\include{discrete_ordinates}
\include{pn_method_and_diffusion}
\include{linearity_and_reciprocity}
\include{the_adjoint_and_perturbation_theory}
\include{variational_methods}
% 

\part{Bibliography and Appendices} 

% Reset to a simple chapter title and format
\renewcommand\printchaptertitle[1]{%
    \begin{tabular}{@{\Large\bfseries\chaptername}@{ }p{1.0cm}!{\quad}p{\textwidth-4cm-2em-4\tabcolsep }}
      \ifNoChapNumber\relax\else\chapnumfont \thechapter\fi
      & \chaptitlefont 
    \end{tabular}
    \NoChapNumberfalse
}
\renewcommand{\chaptername}{Bibliography}
\bibliographystyle{plain}
\bibliography{biblio106}


\appendix
\renewcommand{\chaptername}{Appendix}
\include{nomenclature}
%\include{diffusion}

\end{document}